...
void update_settings_window(ArkanoidSettingsExtended& arkanoid_settings, Arkanoid* arkanoid) {
    {
            ...
            ImGui::SliderInt("Bricks percentage", &arkanoid_settings.bricks_count_percentage, ArkanoidSettings::bricks_min_percentage, ArkanoidSettings::bricks_max_percentage);

            ImGui::Spacing();
            ImGui::SliderInt("Bonus spawn percentage", &arkanoid_settings.bonus_spawn_percentage, ArkanoidSettings::bonus_min_percentage, ArkanoidSettings::bonus_max_percentage);
            ImGui::SliderFloat("Bonus initial speed", &arkanoid_settings.bonus_initial_speed, ArkanoidSettings::bonus_max_speed, ArkanoidSettings::bonus_max_speed);
            ...
            ImGui::Spacing();
            ImGui::SliderFloat("Carriage height", &arkanoid_settings.carriage_height, ArkanoidSettings::carriage_height_min,  ArkanoidSettings::carriage_height_max);
            ...
            ImGui::SliderFloat("Carriage speed", &arkanoid_settings.carriage_speed, ArkanoidSettings::carriage_speed_min, ArkanoidSettings::carriage_speed_max);
...
void update_debug_window(ArkanoidSettingsExtended& arkanoid_settings, Arkanoid* arkanoid, bool& do_arkanoid_update, ImGuiIO& io) {

    ImGui::Begin("Arkanoid Debug");
    ImGui::Checkbox("Debug draw", &arkanoid_settings.debug_draw);

    if (ImGui::Button("Spawn 3 Balls")) {
        arkanoid->apply_bonus(MULTIPLY);
    }

    if (ImGui::Button("+50 Score")) {
        arkanoid->apply_bonus(SCORE);
    }
...
void update_render_game(Arkanoid* arkanoid, ArkanoidDebugData arkanoid_debug_data, ArkanoidSettingsExtended arkanoid_settings, float elapsed_time, ImGuiIO& io, ImDrawList* bg_drawlist, bool do_arkanoid_update) {
    {
...
int main(int, char**)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        update_settings_window(arkanoid_settings, arkanoid);

        // debug window
        update_debug_window(arkanoid_settings, arkanoid, do_arkanoid_update, io);

        ImDrawList* bg_drawlist = ImGui::GetBackgroundDrawList();

        // update/render game
        update_render_game(arkanoid, arkanoid_debug_data, arkanoid_settings, elapsed_time, io, bg_drawlist, do_arkanoid_update);
